...
def agb2_greeks(S, K, r, tau, sigma, a, p, q):
    """
    Griegas de una call europea bajo AGB2, con las fórmulas cerradas del
    capítulo de valoración (parametrización de no arbitraje).

    Notación: kappa = K - alpha;
        F1 = F_{GB2(a,phi,p+1/a,q-1/a)}(kappa),  F2 = F_{GB2(a,phi,p,q)}(kappa),
        barras = colas (1 - F). f1 y f2 son las densidades correspondientes.

    Devuelve dict con delta, gamma, vega, theta, rho. Si K <= alpha (strike
    fuera del soporte) o no existe la varianza base, devuelve NaN.
    """
    alpha, phi = _agb2_risk_neutral_params(S, r, tau, sigma, a, p, q)
    nan = {k: np.nan for k in ("delta", "gamma", "vega", "theta", "rho")}
    if np.isnan(alpha):
        return nan
    ...
    if kappa <= 0:
        # strike por debajo del soporte: call profundamente ITM
        F1 = F2 = 0.0
        f1 = f2 = 0.0
    else:
        F1 = gb2_cdf(kappa, a, phi, p + 1.0 / a, q - 1.0 / a)
        F2 = gb2_cdf(kappa, a, phi, p, q)
        f1 = gb2_pdf(kappa, a, phi, p + 1.0 / a, q - 1.0 / a)
        f2 = gb2_pdf(kappa, a, phi, p, q)

    F1b, F2b = 1.0 - F1, 1.0 - F2  # colas

    # Delta
    delta = F1b + (alpha * disc / S) * (F2b - F1b)

    # Gamma (K > alpha)
    if kappa > 0:
        gamma = (K / S) * (K * (S - alpha * disc) / (S * (K - alpha))) * f1
    else:
        gamma = 0.0

    # Vega
    w = np.exp(sigma ** 2 * tau)
    vega = (S - alpha * disc) * (sigma * tau * w / (w - 1.0)) * (F1b - F2b)

    # Theta (dC/dt): usa lambda
    lam = r + sigma ** 2 * w / (2.0 * (w - 1.0))
    theta = (-r * K * disc
             + disc * (r * K - r * alpha + r * S * np.exp(r * tau)
                       - (S * np.exp(r * tau) - alpha) * lam) * F2
             + disc * (S * np.exp(r * tau) - alpha) * (lam - r) * F1)
...
# ---------------------------------------------------------------------------
# Ejercicio de sensibilidad de McDonald (1991) / Madan (1990)
# ---------------------------------------------------------------------------

def price_grid_from_moments(target_beta1, target_beta2, target_beta3=None,
                            S_grid=None, K=100.0, r=0.10, tau=0.25,
                            sigma2=0.40):
    """
    Dados unos momentos objetivo (beta1, beta2, y opcionalmente beta3), calibra
    la AGB2 por MCE y valora call y put europeas sobre una grilla de precios del
    activo. Pensada para exploración manual e interactiva.

    Si target_beta3 es None, se usa el beta3 de la lognormal de referencia
    (varianza log = sigma2 * tau), de modo que el usuario solo mueva beta1 y beta2.

    Devuelve
    --------
    dict con 'S_grid', 'moneyness', 'bs_call', 'bs_put', 'call', 'put',
    'params' (a, p, q, loss) y 'params_ok' (bool).
    """
    if S_grid is None:
        S_grid = np.arange(90, 111, 2, dtype=float)
    sigma = np.sqrt(sigma2)

    b1_0, b2_0, b3_0 = mcdonald_baseline_moments(sigma2, tau)
    if target_beta3 is None:
        target_beta3 = b3_0

    a, p, q, loss = mce_fit_shape(target_beta1, target_beta2, target_beta3,
                                  a0=0.886, p0=19.788, q0=30.157)

    bs_pairs = [bs_call_put(S, K, r, tau, sigma) for S in S_grid]
    bs_call = np.array([c for c, _ in bs_pairs])
    bs_put = np.array([pp for _, pp in bs_pairs])
    moneyness = S_grid * np.exp(r * tau) / K

    ok = not np.isnan(a)
    if ok:
        pares = [agb2_call_put(S, K, r, tau, sigma, a, p, q) for S in S_grid]
        call = np.array([c for c, _ in pares])
        put = np.array([pp for _, pp in pares])
    else:
        call = np.full_like(S_grid, np.nan)
        put = np.full_like(S_grid, np.nan)
    ...
    return {
        ...
        "params": (a, p, q, loss),
        "params_ok": ok,
        "baseline_moments": (b1_0, b2_0, b3_0),
    }


def greeks_grid(a, p, q, S_grid=None, K=100.0, r=0.10, tau=0.25, sigma2=0.40):
    """
    Calcula las cinco griegas de la call bajo BS y bajo AGB2 (con parámetros
    de forma a, p, q ya calibrados) sobre una grilla de precios del activo.

    Devuelve dict con 'moneyness' y, para cada griega, arrays 'bs' y 'agb2'.
    """
    if S_grid is None:
        S_grid = np.arange(90, 111, 2, dtype=float)
    sigma = np.sqrt(sigma2)
    ...
    nombres = ["delta", "gamma", "vega", "theta", "rho"]
    out = {"moneyness": moneyness}
    for n in nombres:
        out[n] = {"bs": [], "agb2": []}

    for S in S_grid:
        gbs = bs_greeks(S, K, r, tau, sigma)
        gab = agb2_greeks(S, K, r, tau, sigma, a, p, q)
        for n in nombres:
            out[n]["bs"].append(gbs[n])
            out[n]["agb2"].append(gab[n])
    for n in nombres:
        out[n]["bs"] = np.array(out[n]["bs"])
        out[n]["agb2"] = np.array(out[n]["agb2"])
    return out


def mcdonald_baseline_moments(sigma2, tau):
    """
    Momentos centrales estandarizados (beta1, beta2, beta3) de la lognormal
    de referencia del ejercicio. La varianza de los log-retornos bajo la
    dinámica BS es sigma2 * tau, y esos son los momentos que se calibran.

    Para sigma2=0.40, tau=0.25 se obtiene beta1=1.007, beta2=4.856, beta3=15.212,
    coincidiendo con los valores reportados en McDonald (1991).
    """
    return lognormal_standardized_moments(sigma2 * tau)


def mcdonald_sensitivity(sensibilidad, niveles, S_grid=None,
                         K=100.0, r=0.10, tau=0.25, sigma2=0.40):
    """
    Replica el ejercicio de sensibilidad del capítulo de valoración.

    Se parte de los momentos (beta1, beta2, beta3) de la lognormal de
    referencia. Luego se perturba UNO de ellos por niveles porcentuales,
    manteniendo los otros dos fijos, y para cada nivel se recalibran los
    parámetros de forma (a, p, q) por MCE. Con esos parámetros se valora una
    call europea AGB2 (parametrización de no arbitraje) sobre la grilla de
    precios del activo.

    Parámetros
    ----------
    sensibilidad : 'curtosis' (perturba beta2) o 'asimetria' (perturba beta1).
    niveles : lista de perturbaciones porcentuales, p.ej. [0, 25, 50, 75, 100]
              para curtosis, o [0, -25, -50, -75] para asimetría.
    S_grid : grilla de precios del activo (por defecto 90, 92, ..., 110).
    K, r, tau, sigma2 : parámetros del ejercicio (valores de la tesis).

    Devuelve
    --------
    dict con:
        'S_grid'   : la grilla usada.
        'moneyness': F/K para cada S (F = S e^{r tau}).
        'bs_call'  : precios call Black-Scholes (independientes del nivel).
        'bs_put'   : precios put Black-Scholes.
        'call'     : dict {nivel: array de precios call AGB2 en la grilla}.
        'put'      : dict {nivel: array de precios put AGB2 en la grilla}.
        'params'   : dict {nivel: (a, p, q, loss)}.
    """
    if S_grid is None:
        S_grid = np.arange(90, 111, 2, dtype=float)
    sigma = np.sqrt(sigma2)
    ...
    # precios Black-Scholes (columna de referencia, no dependen del nivel)
    bs_pairs = [bs_call_put(S, K, r, tau, sigma) for S in S_grid]
    bs_call = np.array([c for c, _ in bs_pairs])
    bs_put = np.array([p for _, p in bs_pairs])

    # moneyness F/K con F = S e^{r tau}
    moneyness = S_grid * np.exp(r * tau) / K
    ...
    for nivel in niveles:
        factor = 1.0 + nivel / 100.0
        if sensibilidad == "curtosis":
            # se perturba beta2, se fijan beta1 y beta3
            tb1, tb2, tb3 = b1_0, b2_0 * factor, b3_0
        elif sensibilidad == "asimetria":
            # se perturba beta1, se fijan beta2 y beta3
            tb1, tb2, tb3 = b1_0 * factor, b2_0, b3_0
        else:
            raise ValueError("sensibilidad debe ser 'curtosis' o 'asimetria'")

        a, p, q, loss = mce_fit_shape(tb1, tb2, tb3,
                                      a0=0.886, p0=19.788, q0=30.157)
        params[nivel] = (a, p, q, loss)

        if np.isnan(a):
            call[nivel] = np.full_like(S_grid, np.nan)
            put[nivel] = np.full_like(S_grid, np.nan)
            continue
        pares = [agb2_call_put(S, K, r, tau, sigma, a, p, q) for S in S_grid]
        call[nivel] = np.array([c for c, _ in pares])
        put[nivel] = np.array([pp for _, pp in pares])
