...
with tab_dens:
    ...
    with ccol1:
        ...
        st.caption("Parámetros de convergencia:")
        a_conv = st.number_input("a (→ 0)", min_value=0.0001, max_value=2.0,
                                 value=0.3, step=0.05, format="%.4f", key="ln_a")
        q_conv = st.number_input("q (→ ∞)", min_value=1.0, value=200.0, step=10.0,
                                 format="%.1f", key="ln_q")

    # Columna 2: parámetros GB2 generados (solo lectura)
    a_gen, b_gen, p_gen, q_gen = gb2.lognormal_reparam(a_conv, q_conv, mu_ln, sigma2_ln)
    with ccol2:
        st.markdown("**GB2 asociada (generada)**")
        st.metric("a", f"{a_gen:.4f}")
        st.metric("b", f"{b_gen:.4g}")
        st.metric("p", f"{p_gen:.4f}")
        st.metric("q", f"{q_gen:.4f}")

    # Columna 3: gráfica comparativa
    with ccol3:
        # rango basado en la lognormal
        ...
        try:
            gb2_approx = gb2.gb2_pdf(xx, a_gen, b_gen, p_gen, q_gen)
        except Exception:
        ...
        figc.add_trace(go.Scatter(
            x=xx, y=gb2_approx, name="GB2 aproximada",
            line=dict(width=2, dash="dash", color="crimson")
        ))
        figc.update_layout(
            title="Lognormal propuesta vs GB2 asociada",
        ...
        if not np.any(np.isnan(gb2_approx)):
            max_dif = np.max(np.abs(ln_pdf - gb2_approx))
            st.caption(
                f"Discrepancia máxima entre densidades: {max_dif:.4f}. "
                "Reduce $a$ y/o aumenta $q$ para mejorar la aproximación."
            )
