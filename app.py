...
# ===========================================================================
with tab_val:
    st.header("Valoración de opciones europeas: impacto de la asimetría y la curtosis")
    st.write(
        "Réplica del ejercicio de sensibilidad de McDonald (1991) y Madan (1990). "
        "Se parte de una lognormal de referencia y se perturba uno de sus momentos "
        "(curtosis β₂ o asimetría β₁), manteniendo los otros fijos. Para cada nivel "
        "se recalibran los parámetros de forma $(a,p,q)$ de la AGB2 mediante el "
        "método MCE y se valoran las opciones. El eje horizontal es la "
    ...
    # Elección de la sensibilidad a estudiar
    sens = st.radio(
        "Sensibilidad a estudiar",
        ["Curtosis (β₂)", "Asimetría (β₁)"],
        horizontal=True,
    )

    if sens.startswith("Curtosis"):
        sensib = "curtosis"
        niveles = [0, 25, 50, 75, 100]
        nota = ("Se incrementa β₂ en 0%, 25%, 50%, 75% y 100%, manteniendo β₁ y "
                "β₃ fijos. Un aumento de curtosis (colas más pesadas) reduce el "
                "precio de las opciones ATM, con el efecto acentuándose fuera del dinero.")
    else:
        sensib = "asimetria"
        niveles = [0, -25, -50, -75]
        nota = ("Se reduce β₁ en 0%, −25%, −50% y −75%, manteniendo β₂ y β₃ fijos, "
                "desplazando la distribución hacia un sesgo izquierdo. La asimetría "
                "afecta poco las opciones ATM pero cada vez más al alejarse del strike.")

    st.caption(nota)

    # Cálculo cacheado del ejercicio
    @st.cache_data(show_spinner="Calibrando por MCE y valorando opciones...")
    def _ejercicio(sensib, niveles, K, r, tau, sigma2):
        return gb2.mcdonald_sensitivity(
            sensib, niveles, K=K, r=r, tau=tau, sigma2=sigma2
        )

    res = _ejercicio(sensib, tuple(niveles), K_ex, r_ex, tau_ex, sigma2_ex)
    mny = res["moneyness"]

    def _etiqueta(nivel):
        signo = "+" if nivel >= 0 else ""
        return "Lognormal (BS)" if nivel == 0 else f"AGB2 {signo}{nivel}%"
    ...
    with col_call:
        ...
        for i, nivel in enumerate(niveles):
            figc.add_trace(go.Scatter(
                x=mny, y=res["call"][nivel], name=_etiqueta(nivel),
    ...
    with col_put:
        ...
        for i, nivel in enumerate(niveles):
            figp.add_trace(go.Scatter(
                x=mny, y=res["put"][nivel], name=_etiqueta(nivel),
                line=dict(width=2, color=colores[i % len(colores)]),
            ))
    ...
    st.caption(
        "La línea vertical gris marca el nivel ATM (F/K = 1). La curva Black-Scholes "
        "(punteada) es independiente de la curtosis y la asimetría por construcción; "
        "las curvas AGB2 muestran cómo la flexibilidad distribucional modifica los "
        "precios respecto a la lognormal."
    )
